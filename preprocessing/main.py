...
def clip(data_path, output_path, sd_factor, output_type, rescale):
    ...
    out_path = output_path / f'{data_path.stem}_clipped.{output_type}'
...
@click.option('-ot', '--output-type', default='h5', help='Output file type, h5 or tiff')
def gaussian_blur(data_path, output_path, sigma, output_type):
    data_path = Path(data_path)
    output_path = Path(output_path)
    handle = get_handler(data_path)(file=data_path)
    data = ip.denoise(
    ...
    out_path = output_path / f'{data_path.stem}_denoised.{output_type}'
    output_handler = get_handler(out_path)
    output_handler.write(
        out_path,
        data,
